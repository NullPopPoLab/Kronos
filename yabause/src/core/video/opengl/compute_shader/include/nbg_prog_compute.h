...
"uint ReadCramValue(uint index, uint line) {\n"
"   uint colorval = 0u; \n"
"   colorval = cram[(index >> 1) + 0x1000u * line]; \n"
"   if ((index & 0x02u) != 0u) { colorval >>= 16; } \n"
"   return colorval & 0xFFFFu; \n"
"}\n"
"vec4 ReadCramColor(uint index, uint line, uint colorcalc) {\n"
"   uint tmp1 = ReadCramValue((index&0xFFCu), line);\n"
"   uint tmp2 = ReadCramValue((index&0xFFCu)+2, line);\n"
"   vec4 color = vec4(0.0);\n"
"   color.r = float(tmp2 & 0xFFu)/255.0;\n"
"   color.g = float((tmp2 >> 8u) & 0xFFu)/255.0;\n"
"//We are passing the MSB CC as LSB ob blue color\n"
"   color.b = float(((tmp1) & 0xFEu)|colorcalc)/255.0;\n"
"   return color;\n"
"}\n"
"uint get_cram_msb(uint index, uint line) { \n"
"   uint tmp1 = ReadCramValue((index&0xFFC), line);\n"
"   return (tmp1 & 0x8000u); \n"
"}\n";
...
"void main()\n"
"{\n"
"uint idCmd = gl_WorkGroupID.x * 11;\n"
"ivec2 size = imageSize(outSurface);\n"
"uint cellw = cmd[idCmd+8];\n"
"uint cellh = cmd[idCmd+9];\n"
"uvec2 pos = uvec2(gl_LocalInvocationID.x, gl_LocalInvocationID.y);\n"
"if ((cmd[idCmd+10] & 1)!=0) pos.x = (cellw - 1)-pos.x;\n"
"if ((cmd[idCmd+10] & 2)!=0) pos.y = (cellh - 1)-pos.y;\n"
"ivec2 texel = ivec2(cmd[idCmd]+pos.x,(size.y-1)-(cmd[idCmd+1]+pos.y));\n"
