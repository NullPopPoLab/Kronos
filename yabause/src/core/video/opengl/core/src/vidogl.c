...
static void requestDrawCellOrderCPU(vdp2draw_struct * info, YglTexture *texture, Vdp2* varVdp2Regs, int order) {
...
vdp2draw_struct infoNBG0;
static void Vdp2DrawNBG0(Vdp2* varVdp2Regs) {
  vdp2draw_struct* info = &infoNBG0;
  memset(info, 0, sizeof(vdp2draw_struct));
  YglTexture texture;
  YglCache tmpc;
  u32 char_access = 0;
  u32 ptn_access = 0;
...
// NBG0 draw
      ...
      if (info->coordincx != 1.0f || info->coordincy != 1.0f || VDPLINE_SZ(info->islinescroll)) {
        ...
        if (info->islinescroll) // Nights Movie
        {
          ...
          if (VIDCore != NULL && VIDCore->id == VIDCORE_OGL) {
            ...
            CSDrawNBGBitmapScroll(info, _Ygl->rwidth, _Ygl->rheight);
          }
        }
        else {
          yy = info->y;
          while (yy + info->y < _Ygl->rheight)
          {
            ...
            while (xx + info->x < _Ygl->rwidth)
            {
              if (VIDCore != NULL && VIDCore->id == VIDCORE_OGL) {
                ...
                //CS use case
                if (info->islinescroll) {
                  CSDrawNBGBitmapScroll(info, info->cellw, info->cellh);
                } else {
                  DrawCellOrderCS(info, xx, yy);
                }
              }
              xx += info->cellw;
            }
    ...
    else
    {
      if (info->islinescroll) {
        ...
        // if (VIDCore != NULL && VIDCore->id != VIDCORE_CS)
          Vdp2DrawMapTest(info, &texture, varVdp2Regs);
        // else
          // CSDrawNBGTileMap(&info, &_Ygl->screen_fbotex[NBG0], varVdp2Regs);
      }
    }
  executeDrawCell();
}
...
//////////////////////////////////////////////////////////////////////////////
vdp2draw_struct infoNBG1;
static void Vdp2DrawNBG1(Vdp2* varVdp2Regs)
{
  vdp2draw_struct* info = &infoNBG1;
  memset(info, 0, sizeof(vdp2draw_struct));
  YglTexture texture;
  YglCache tmpc;
  u32 char_access = 0;
...
//////////////////////////////////////////////////////////////////////////////
vdp2draw_struct infoNBG3;
static void Vdp2DrawNBG3(Vdp2* varVdp2Regs)
{
  vdp2draw_struct* info = &infoNBG3;
  memset(info, 0, sizeof(vdp2draw_struct));
  YglTexture texture;
  info->idScreen = NBG3;
  info->dst = 0;
